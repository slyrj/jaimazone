...
function renderCard(p, i) {
  const cat   = DATA.categories.find(c => c.id === p.categorie)?.label || p.categorie;
  const promo = p.prix_barre ? `<div class="price-old">${formatPrix(p.prix_barre)}</div>` : '';
  const pts   = p.points.slice(0, 3).map(t => `<span class="card-point">${t}</span>`).join('');
  return `
  <article class="product-card" style="animation-delay:${Math.min(i * .06, .6)}s">
    <div class="card-rank ${p.rang <= 3 ? 'top3' : ''}">${p.rang}</div>
    ${p.badge ? `<div class="card-badge badge-${p.badge_c}">${p.badge}</div>` : ''}
    <div class="card-image">${p.icone}</div>
    <div class="card-body">
      ...
      <div class="card-rating">
        <span class="stars">${renderEtoiles(p.note)}</span>
        <span class="rating-num">${p.note}</span>
        <span class="rating-count">(${formatNombre(p.avis)} avis)</span>
      </div>
    </div>
    <div class="card-footer">
      <div class="card-price">
        <div class="price-current">${formatPrix(p.prix)}</div>
        ${promo}
      </div>
...
function getList() {
  ...
  if (query) {
    ...
    l = l.filter(p => p.titre.toLowerCase().includes(q) || p.desc.toLowerCase().includes(q)
...
function buildFilters() {
  document.getElementById('filtersContainer').innerHTML = DATA.categories.map(c =>
...
function buildFooterCats() {
  ...
  if (!ul) return;
  ul.innerHTML = DATA.categories.filter(c => c.id !== 'tout').map(c =>
    `<li><a href="#" data-cat="${c.id}">${c.label}</a></li>`
  ).join('');
  ul.addEventListener('click', e => {
    ...
    document.querySelectorAll('.filter-btn').forEach(b => b.classList.toggle('active', b.dataset.cat === filter));
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  buildFooterCats();
  render();
  animerCompteur(document.getElementById('counterProduits'), DATA.produits.length);
